...
def handle_connection(connection: socket.socket, addr: tuple[str, int]):
    try:
        ...
        if not (RECEIVER_IP, int(RECIEVER_PORT)) in RUNNING_PROCESSES:
        ...
        error_message = f"NO AVAILABLE DEVICES"
...
def send_to_receiver(RECEIVER_IP: str, RECEIVER_PORT: int):
    ...
    RUNNING_PROCESSES.append((RECEIVER_IP, RECEIVER_PORT))
    ...
    try:
        ...
        RUNNING_PROCESSES.remove((RECEIVER_IP, RECEIVER_PORT))
    ...
    try:
        while True:
            ...
            new_thread = threading.Thread(
                target=handle_connection, args=(connection, addr)
            )
            new_thread.start()
